...
1. Run `node scripts/integrity-check.mjs` from the repo root (or `node scripts/integrity-check.mjs <section>` for line-limit | asset-bundle | stale-binary | diff-hygiene | adapter-matrix | doc-sync).
