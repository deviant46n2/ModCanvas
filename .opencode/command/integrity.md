...
1. Run `node scripts/integrity-check.mjs` from the repo root (or `node scripts/integrity-check.mjs <section>` for line-limit | asset-bundle | stale-binary | diff-hygiene).
2. **VIOLATION entries are new violations** — the gate is clean on the known debt (parked). For each violation, triage with the student like any debt:
   - pay now (split the file / remove the asset / rebuild) — or
...
4. Teach the *why* behind each check (AGENTS.md rule it encodes) so the student can judge, not just run: 300-line = size is a tripwire for extraction; asset-bundle = no game-derived image bytes in the bundle; stale-binary = the binary embeds src + frontend; diff-hygiene = whitespace lies about structure.
5. Verify the tool itself is honest: it must obey the repo's rules (docs synced in docs/tooling.md; run `node --test scripts/integrity-check.test.mjs` after any engine change).
