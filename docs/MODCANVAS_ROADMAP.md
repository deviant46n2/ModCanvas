...
| 7 | **Dep-gate the green check (s54-B) — RULED (s55): warn persistently, never gate** | **DONE (s55)** — missing required deps surface as a persistent, NON-blocking warning in Pack Health (Mods section, `checkMissingDeps` + `depIssues` in the pack-health store, fed by every compat-check site). Launch stays open — the user may legitimately not want to install a mod right now (student ruling). The network-dependence question resolved: the compat check already degrades to no-claim on fetch failure; the store is the materialized cache. The core-mod gate (s53: ModCanvas's OWN editors' deps, FTB Quests + KubeJS) remains blocking and separate. | The s54-A dep loop (inline one-click installs) closed the *action* loop; this closes the *honesty* loop — a pack with an unresolved required dep is never silently blessed, but also never blocked over it. |
