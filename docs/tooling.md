...
| `stale-binary` | binary embeds src + frontend; stale binary serves old behavior | per-binary scoping: dev binary vs `src-tauri/src` only (frontend hot-reloads in `pnpm dev` — F4 fix); release binary vs backend + frontend; **companion jar** vs `workbench-companion-neoforge-1.21/src` (added s61 — the companion is a separate artifact with its own rebuild/deploy loop; a stale bundled jar silently serves old render/registry behavior, and the app binaries cannot see it). Either may be parked with a written reason |
...
| `doc-sync` | docs are code (AGENTS.md) | commits in the last 10 that touched code but no doc — **candidates**, maintainer judges (refactors/reverts are legitimately doc-less); never a gate. Code paths include `workbench-companion-neoforge-1.21/src` (added s61 — companion Java changes now surface, after the parity fix invalidated `engine-renders.md` invisibly). Judged commits (`docSync.judgments` in the rules, written reason) are info, not candidates. A candidate that **ages out unjudged** does not vanish — the health report transitions it to visible P2 work via `.doc-sync-state.json` (s30: the 65c1fe8 failure — an unjudged candidate sat invisible for 5 sessions) |
...
| `build-smoke` | a frontend that cannot build cannot serve behavior claims (s34: the bd4016b class — a split broke CSS barrel imports tsc cannot see; the app ran with missing styles for ~2.5h until accfab5) | runs the frontend's OWN `build` script (`tsc -b && vite build`) with `node_modules/.bin` on PATH (the same binary resolution the repo uses via its package manager); violation on non-zero exit with the error tail. Skippable via `buildSmoke.skip` in the rules for dependency-less environments |
