...
pub fn scan_jar_for_textures(jar_path: &Path) -> anyhow::Result<Vec<TextureEntry>> {
    ...
    for i in 0..archive.len() {
        ...
        let rest = &name[format!("assets/{}/textures/", namespace).len()..];

        let item_id = if rest.starts_with("item/") {
            let item_name = rest.strip_prefix("item/").unwrap_or(rest).strip_suffix(".png").unwrap_or(rest);
            Some(format!("{}:{}", namespace, item_name))
        } else if rest.starts_with("block/") {
            let block_name = rest.strip_prefix("block/").unwrap_or(rest).strip_suffix(".png").unwrap_or(rest);
            Some(format!("{}:{}", namespace, block_name))
        } else if rest.starts_with("model/") {
            let model_name = rest.strip_prefix("model/").unwrap_or(rest).strip_suffix(".png").unwrap_or(rest);
            Some(format!("{}:{}", namespace, model_name))
        } else {
            None
        };
