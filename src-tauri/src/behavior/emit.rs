...
use crate::behavior::{compile::compile_to_kubejs, Behavior};
use crate::path_safety::{atomic_write_str, validate_under_root};
use std::path::Path;

/// The dedicated script file for all ModCanvas-authored behaviors.
pub const BEHAVIORS_SCRIPT_REL: &str = "kubejs/server_scripts/modcanvas_behaviors.js";

/// Compile + write all behaviors to the instance's KubeJS scripts dir.
///
...
pub fn emit_behavior_scripts(
    ...
    let target = validate_under_root(&Path::new(project_path), BEHAVIORS_SCRIPT_REL)?;
