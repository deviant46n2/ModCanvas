#!/usr/bin/env node
// health-report.mjs — repo health thermometer (the debt ledger, 2026-08-09).
//
// Score = "manageable debt load" (the student's definition), NOT code quality.
...
//   parked     — near-zero: known debt WITH a written reason (P4: good parks
//                must not be punished — the s22 lesson)
...
// file's `health` block; this script never grows per-class (the same seam the
// integrity engine uses). Report-only: exit 0 always (no gate — a report that
...
// Health rules live in their own data file (split from integrity-rules.json
// when the seeded ledger + since fields pushed it past its own 300-line
// limit — the s22 meta-rule: the tooling obeys the repo's rules). New failure
// classes = new ledger entries HERE, never a script edit.
...
// Local calendar date (YYYY-MM-DD): toISOString is UTC and rolls over at
// 6-7pm local (CST) — a run at 19:xx would be logged as tomorrow.
const localDate = () => {
  const d = new Date()
...
export function computeScore(results, health) {
  ...
  for (const item of health.ledger ?? []) {
    ...
    breakdown[`ledger:${item.id}`] = { violations: 0, candidates: 0, parked: 0, deduction: w }
    deduction += w
  }
  return { score: Math.max(0, Math.round(100 - deduction)), breakdown, deduction: +deduction.toFixed(1) }
}

/** Last commit timestamp of a path (ISO-8601 local, e.g. 2026-08-09T19:25:56-05:00),
 *  or null when the path is untracked/absent. Git is truth: a park's tripwire
 *  ("revisit on next touching change") fires only when the file was ACTUALLY
 *  touched after it was parked — a park whose contract hasn't fired is not work. */
...
export function rankWork(results, health, root) {
  // What to work on next: new violations first (the exit-1 class), then
  // ledger items by priority, then candidates (judgment), then parked items
  // whose tripwire has FIRED (file touched since parked).
  const out = []
  for (const section of results) {
