// integrity-select.test.mjs — section-selection + CLI arg parsing (s65).
//
// The s65 CI matrix needs "all sections except build-smoke" on Windows
// (build-smoke spawns `sh`, Linux-only). These tests lock the selection
// semantics so a future CLI change cannot silently narrow or widen what
// a platform-aware run executes.
...
test('selectSections: skip build-smoke → all except it (the Windows CI shape)', () => {
...
test('selectSections: named sections run exactly those', () => {
  const sel = selectSections(sections, ['line-limit', 'asset-bundle'])
  assert.deepEqual(sel.map((s) => s.name), ['line-limit', 'asset-bundle'])
})
