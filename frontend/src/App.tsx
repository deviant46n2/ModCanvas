...
function App() {
  ...
  useEffect(() => {
    if (!showNewInstance || newInstanceLoader === 'vanilla') {
      return
    }
    let cancelled = false
    invoke<string>('resolve_mc_loader_version', {
      loader: newInstanceLoader,
      mcVersion: newInstanceMcVersion,
      requestedVersion: null,
    }).then((v) => {
      if (!cancelled && v) {
        setNewInstanceLoaderVersion(v)
      }
    }).catch(() => {})
    return () => { cancelled = true }
  }, [showNewInstance, newInstanceMcVersion, newInstanceLoader])
  ...
  async function launchInstance(id: string) {
    ...
    const unlisten = await listen<{ phase: string; message: string }>('mc-launch-progress', (event) => {
      setLaunchProgress(event.payload.message)
      if (event.payload.phase === 'done' || event.payload.phase === 'error') {
        setIsLaunching(false)
        loadInstances()
        if (event.payload.phase === 'error') setLaunchError(event.payload.message)
          ...
          <div className="instance-workspace">
            ...
            {launchProgress && (
              <div className="launch-progress">
                <div className="progress-phase">{launchProgress}</div>
              </div>
            )}
