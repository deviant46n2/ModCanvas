...
| Panel/border/background styling from theme selectors | 🟡 | Background is now FULLY resolved (s64, clean-room FTB Quests v2101.1.31 + FTB-Library v2101.1.35): window-space draw (`FTBQuestsTheme.drawGui`), `tile_size` present → repeat at that px / absent → stretch, `color=` = vertex tint (alpha-modulating opacity — ImageIcon.draw), chapter→global fallback, default = `background_squares.png` tiled 64px + `#DCFFFFFF` from the shipped `ftb_quests_theme.txt`. The 1px quest-pane frame uses the DEFAULT `widget_border` (#1B1D1E, `hollow_rectangle`) but is not yet driven by parsed per-theme selectors; other panel/checkmark selectors still hardcoded |
| Chapter-open viewport framing | ✅ | Default zoom matches the game (16 → 28px/unit, `QuestScreen.java:60` + pitch math `zoom*(3/2+spacing/4)`), chapter opens centered (`QuestPanel.resetScroll`, centered on content), and the whole default view (canvas zoom + background tiles) scales by the instance's `guiScale` from `options.txt` (fallback 1) so the editor matches the player's actual game look (verified s64 against a guiScale-3 instance). Parked (game-fidelity follow-ups): persisted-viewport restore on reopen (`restorePersistedScreenData`), chapter autofocus `scrollTo`, wheel-zoom clamp 4–28 with center preservation |
